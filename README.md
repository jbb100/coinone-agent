...
국면 모델은 `config.yaml`의 `strategy.regime_model`로 선택합니다.
...
**valuation 모델 (가치 앵커 — 저평가 매집 / 고평가 분배)** 🆕
```
R = BTC 현재가 / 200주 이동평균

💎 DEEP_VALUE   (R < 1.0):       암호화폐 75% — 사이클 바닥권 최대 매집
📥 ACCUMULATION (1.0 ≤ R < 1.4): 암호화폐 65% — 저평가 적극 매집
➡️ NEUTRAL      (1.4 ≤ R < 2.0): 암호화폐 50%
📤 DISTRIBUTION (2.0 ≤ R < 2.8): 암호화폐 35% — 고평가 단계적 분배
🔥 EUPHORIA     (R ≥ 2.8):       암호화폐 25% — 과열 최대 분배

- 경계 ±5% 히스테리시스로 휩쏘 방지
- 국면 전환 시 회당 최대 10%p씩 단계 이동 (일괄 매매 방지)
- 백테스트 검증 후 전환 권장 (docs/TRADING_LOGIC_IMPROVEMENT_PLAN.md 참고)
```

**공통 안전 원칙 (Real-Data-Only)**
- 시장 데이터(200주 MA, 시세)를 얻지 못하면 **판단하지 않고 거래를 중단**합니다
  (임의 기본값으로 대체하지 않음)
- 공포탐욕지수는 alternative.me 실제 API만 사용하며, 조회 불가 시 중립 처리합니다
- 기회적 매수분은 30일간 리밸런싱 매도에서 면제됩니다 (AllocationArbiter)
- 시장 과열 시 OpportunisticSeller가 보유분을 단계적으로 익절해 KRW를 확보합니다
