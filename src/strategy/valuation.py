...
# -50%까지는 유지, -70%에서 완전 편출. 연속 함수라 경계 노이즈가
# 급격한 편출입 매매를 만들지 않는다 (틸트와 동일한 설계 원칙).
# 2026-08-01 멀티자산 일봉 백테스트: 구 트리거(-30%~-50%)는 정상 알트
# 사이클에도 상시 발동 → 가중치가 매일 드리프트하며 회전율 2배(비용
# 원금의 10%)·수익 -0.16x. 심화 트리거는 평시 factor=1.0으로 고정되어
# 회전율 문제를 제거하고, LUNA형 구조적 붕괴 보험만 남긴다.
_RS_REL_ANCHORS = (-0.7, -0.5)
