...
class MarketDataProvider:
    ...
    def get_btc_200w_ma(self, fallback_to_current_price: bool = True) -> Tuple[float, str]:
        ...
        try:
            ...
            # 2. yfinance에서 실시간 계산
            try:
                ma_200w = self._calculate_200w_ma_from_yfinance()
                if ma_200w and ma_200w > 0:
                    # 계산된 값 캐싱
                    if self.db_manager:
                        self._cache_200w_ma(ma_200w)
                    logger.info(f"yfinance에서 200주 이동평균 계산: {ma_200w:.2f}")
                    return ma_200w, "yfinance"
            except Exception as e:
                logger.warning(f"yfinance 200주 이동평균 계산 실패: {e}")
    ...
    def _calculate_200w_ma_from_yfinance(self) -> Optional[float]:
        """
        yfinance를 사용하여 200주 이동평균 계산
        
        Returns:
            200주 이동평균 또는 None
        """
        try:
            logger.info("yfinance를 통한 BTC 200주 이동평균 계산 시작")
            
            # BTC-USD 데이터 가져오기 (3.5년 이상의 주간 데이터 필요)
            btc = yf.Ticker("BTC-USD")
            
            # 200주 = 약 1400일 (여유분 포함하여 5년 데이터 요청)
            end_date = datetime.now()
            start_date = end_date - timedelta(days=1800)  # 5년
            ...
            if hist.empty:
                logger.error("yfinance에서 BTC 데이터를 가져올 수 없습니다")
                return None
            
            logger.info(f"yfinance에서 {len(hist)}주의 BTC 데이터 수집")
            
            # 200주 이동평균 계산
            if len(hist) < 200:
                logger.warning(f"데이터가 부족합니다. 필요: 200주, 보유: {len(hist)}주")
                # 보유한 데이터로 최대한 계산
                ma_period = min(len(hist), 200)
                ma_200w = hist['Close'].rolling(window=ma_period).mean().iloc[-1]
                logger.info(f"{ma_period}주 이동평균으로 계산: {ma_200w:.2f}")
            else:
                # 정확한 200주 이동평균 계산
                ma_200w = hist['Close'].rolling(window=200).mean().iloc[-1]
                logger.info(f"정확한 200주 이동평균 계산: {ma_200w:.2f}")
            ...
            logger.error(f"yfinance 200주 이동평균 계산 실패: {e}")
            return None
    
    def _get_current_btc_price(self) -> Optional[float]:
        """
        현재 BTC 가격 조회 (다양한 소스 시도)
        ...
        try:
            ...
            with self.db_manager.get_connection() as conn:
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?)
                """, ("BTC", ma_value, datetime.now().isoformat(), "yfinance"))
