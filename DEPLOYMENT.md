...
거래소는 전략의 어떤 가드도 막을 수 없는 단일 실패점이다 (FTX 사례:
시장이 아니라 거래소가 최대 손실 원인). 원칙: **거래소에는 리밸런싱에
필요한 유동성만 둔다.**

1. `config.yaml`의 `security.exchange_exposure_cap_krw`에 거래소 보관
   상한을 설정한다. 총평가액이 상한을 넘으면 데일리 브리핑에 초과분과
   함께 경고가 표시된다.
2. 상한 설정 기준(권장): 다음 1~2회 리밸런싱/DCA에 필요한 KRW + 크립토
   일부. 나머지 크립토는 개인 하드웨어 월렛으로 출금.
3. 경고 수신 시 절차: 코인원 → 개인 월렛 출금 (주소 화이트리스트 사전
   등록, 소액 테스트 출금 후 본출금). 출금 내역은 별도 기록 —
   시스템 잔고 조회에서 사라지므로 총자산 추적이 필요하면 수기 관리.
4. 콜드월렛 보유분은 시스템 리밸런싱 대상에서 자연히 제외된다 —
   장기 보유분(팔 일 없는 물량)부터 이전하는 것이 전략 왜곡이 적다.
